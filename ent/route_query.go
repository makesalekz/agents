...
func (rq *RouteQuery) QueryPoints() *RoutePointQuery {
	query := (&RoutePointClient{config: rq.config}).Query()
...
func (rq *RouteQuery) IDs(ctx context.Context) (ids []int64, err error) {
...
func (rq *RouteQuery) Clone() *RouteQuery {
	...
	return &RouteQuery{
		...
		withPoints: rq.withPoints.Clone(),
...
func (rq *RouteQuery) WithPoints(opts ...func(*RoutePointQuery)) *RouteQuery {
	query := (&RoutePointClient{config: rq.config}).Query()
...
func (rq *RouteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Route, error) {
	...
	if query := rq.withPoints; query != nil {
		if err := rq.loadPoints(ctx, query, nodes,
			func(n *Route) { n.Edges.Points = []*RoutePoint{} },
			func(n *Route, e *RoutePoint) { n.Edges.Points = append(n.Edges.Points, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RouteQuery) loadPoints(ctx context.Context, query *RoutePointQuery, nodes []*Route, init func(*Route), assign func(*Route, *RoutePoint)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(routepoint.FieldRouteID)
	}
	query.Where(predicate.RoutePoint(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(route.PointsColumn), fks...))
	}))
...
func (rq *RouteQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(route.Table, route.Columns, sqlgraph.NewFieldSpec(route.FieldID, field.TypeInt64))
	_spec.From = rq.sql
